...
router.get('/', (req, res) => {
  try {
    ...
    const spaces = Space.getAllByUser(userId);
    res.json(spaces);
  } catch (error) {
...
router.put('/sort', (req, res) => {
  try {
    ...
    Space.updateSortOrder(spaceOrders);
...
router.get('/:id', (req, res) => {
  try {
    const space = Space.getById(req.params.id);
    if (!space) {
...
router.post('/', (req, res) => {
  try {
    ...
    const space = Space.create(userId, name);
...
router.put('/:id', (req, res) => {
  try {
    ...
    const space = Space.update(req.params.id, name);
    if (!space) {
      return res.status(404).json({ error: 'Space not found' });
    }
    res.json(space);
